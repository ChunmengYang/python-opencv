...
# 模版匹配
def matchTemplateDemo(img_path, template_path):
    img = cv2.imread(img_path, 1)
    img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
# 关键点检查
def keypointsDemo(img_path, face_path):
    img = cv2.imread(img_path, 1)
...
# 凸包、凸缺陷检测
def hullAndDefectsDemo(img_path):
    ...
    # 轮廓检测
    contours_map, contours, hierarchy = cv2.findContours(frame_morph, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  
